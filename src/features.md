...
*   **AI Task Explanations (Employee):**
    *   **Location:** Employee Dashboard -> My Mission
    *   **Functionality:** Employees can click a "Why?" button next to any task. The AI (`explain-task-importance-flow.ts`) then provides a motivational, context-aware explanation, connecting the task to broader goals like guest safety or team efficiency.
