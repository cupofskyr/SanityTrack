...
### Theme 5: Financial Operations & Integration

*   **Intelligent Financial Integration (Owner):**
    *   **Location:** Owner Dashboard -> System Administration -> Financials.
    *   **Functionality:** Provides a secure, OAuth 2.0-based connection to QuickBooks Online. This enables automated, AI-assisted expense reporting.
    *   **How it Works:** Owners connect their QBO account once. Employees and managers can then submit expenses through a guided, multi-step wizard. The AI assists by categorizing the expense based on its description and the company's Chart of Accounts, which is synced from QBO. Approved expenses are then seamlessly created as `Purchase` objects in QuickBooks, with receipts attached. The system is designed to be resilient, handling API errors gracefully and maintaining data integrity between the two platforms.

### Theme 6: Knowledge, Training & Administration
...
*   **Key AI Techniques Used:**
    ...
    *   **Multi-modal Analysis:** Features like invoice scanning and food quality audits provide the AI with both text (images) and visual information, allowing it to "see" and reason about them.
