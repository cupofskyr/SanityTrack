...
export default function HealthDeptDashboard() {
  ...
  const [inspectorInvestigationNotes, setInspectorInvestigationNotes] = useState('');
  ...
  const handleOpenInvestigateDialog = async (report: Report) => {
    ...
    setInspectorInvestigationNotes(''); // Reset notes for new investigation
  ...
  const handleSaveInvestigationNotes = () => {
    if (!selectedReportForInvestigation) return;
    // In a real app, this would save to a DB.
    toast({ title: "Notes Saved", description: "Your notes on this investigation have been logged." });
    setIsInvestigateDialogOpen(false);
  };
  
  const handleCreateMandatoryTask = () => {
    if (!selectedReportForInvestigation) return;
    // In a real app, this would create a task in the owner's queue.
    toast({ title: "Mandatory Task Created", description: `A task has been assigned to the owner of ${selectedReportForInvestigation.location}.` });
    setIsInvestigateDialogOpen(false);
  };
