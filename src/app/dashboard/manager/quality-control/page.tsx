...
import { Loader2, Sparkles, CheckCircle, XCircle, PlusCircle, Pencil, Trash2, Send } from 'lucide-react';
...
export default function QualityControlPage() {
    ...
    const handleAudit = async () => {
        if (!selectedStandard) {
             toast({ variant: 'destructive', title: "No Standard Selected", description: "Please select a golden standard item to compare against." });
            return;
        }
        if (!auditPhotoUri) {
    ...
    const handleCreateTask = () => {
        ...
        // In a real app, this would save to a database. Here we simulate.
        console.log({
            assignee: taskAssignee,
            title: `Quality Control Follow-up: ${selectedStandard?.name}`,
            description: `Audit Score: ${auditResult.score}/10. Feedback: ${auditResult.feedback}. Deviations: ${auditResult.deviations.join(', ')}`
        });
    ...
    return (
        <div className="space-y-6">
                            ...
                            {standardItems.length > 0 ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={2} className="text-center h-24 text-muted-foreground">
                                        No golden standard items have been created yet.
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
                ...
                <CardContent className="grid md:grid-cols-2 gap-8 items-start">
                    <div className="space-y-4">
                        ...
                        <Select
                            ...
                            onValueChange={(name) => {
                                setSelectedStandard(standardItems.find(item => item.name === name)!);
                         ...
                         <Dialog open={isTaskDialogOpen} onOpenChange={setIsTaskDialogOpen}>
                            <DialogTrigger asChild>
                                <Button variant="secondary">
                                    <PlusCircle className="mr-2 h-4 w-4" /> Create Follow-up Task
                                </Button>
                            </DialogTrigger>
                            <DialogContent>
                                <DialogHeader>
                                ...
                                <div className="py-4 space-y-4">
                                    <div className="grid gap-2">
                                        ...
                                        <Textarea 
                                            readOnly 
                                            defaultValue={`Quality issue on "${selectedStandard?.name}". Score: ${auditResult.score}/10. Feedback: ${auditResult.feedback}`}
                                            rows={4}
                                        />
                                    </div>
                                    <div className="grid gap-2">
                                ...
                                <DialogFooter>
                                    <Button onClick={handleCreateTask} disabled={!taskAssignee}>
            ...
            {/* Dialog for Adding/Editing a Standard Item */}
            <Dialog open={isMenuDialogOpen} onOpenChange={setIsMenuDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{currentItem?.id ? "Edit" : "Add"} Golden Standard</DialogTitle>
                        <DialogDescription>
                            {currentItem?.id ? "Update the details for this standard." : "Add a new reference item for quality checks."}
                        </DialogDescription>
                    </DialogHeader>
                    {currentItem && (
                         <form onSubmit={handleSaveMenuItem}>
                            <div className="grid gap-4 py-4">
                                ...
                                <div className="grid gap-2">
                                    ...
                                    <PhotoUploader 
                                        initialPreview={currentItem.imageUrl ? { url: currentItem.imageUrl, name: "reference.png" } : undefined}
                                        onPhotoDataChange={(dataUrl) => setCurrentItem({...currentItem, imageUrl: dataUrl || ''})} 
                                    />
                                </div>
                            </div>
                            <DialogFooter>
            ...
            {/* Dialog for Deleting a Standard Item */}
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Are you sure?</AlertDialogTitle>
                        <AlertDialogDescription>
                            This action cannot be undone. This will permanently delete the golden standard for <span className="font-semibold">"{itemToDelete?.name}"</span>.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
